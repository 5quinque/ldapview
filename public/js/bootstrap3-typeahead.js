! function(a, b) {
    ...
    b.prototype = {
        ...
        show: function() {
            ...
            var e = a(b).parent().hasClass("dropup"),
                f = e ? "auto" : c.top + c.height + d,
                g = a(b).hasClass("dropdown-menu-right"),
                h = g ? "auto" : c.left;
            return b.css({
                top: f,
                left: h
        ...
        highlighter: function(b) {
            var c, d, e, f, g = a("<div></div>"),
                h = this.query,
                i = b.toLowerCase().indexOf(h.toLowerCase()),
                j = h.length;
            if (0 === j) return g.text(b).html();
            for (; i > -1;) c = b.substr(0, i), d = b.substr(i, j), e = b.substr(i + j), f = a("<strong></strong>").text(d), g.append(document.createTextNode(c)).append(f), b = e, i = b.toLowerCase().indexOf(h.toLowerCase());
        ...
        render: function(b) {
            ...
            return a.each(b, function(a, c) {
                ...
                return b = a(c.options.item).data("value", f), b.find("a").html(c.highlighter(g, f)), g == d.$element.val() && (b.addClass("active").find('a').addClass('active'), d.$element.data("active", f), e = !0), b[0]
            }), this.autoSelect && !e && (b.filter(":not(.dropdown-header)").first().addClass("active").find('a').addClass('active'), this.$element.data("active", b.first().data("value"))), this.$menu.html(b), this
        },
        ...
        next: function(b) {
            var c = this.$menu.find(".active").removeClass("active"),
                d = c.next();
            d.length || (d = a(this.$menu.find("li")[0])), d.addClass("active").find('a').addClass('active');
        },
        prev: function(a) {
            var b = this.$menu.find(".active").removeClass("active"),
                c = b.prev();
            c.length || (c = this.$menu.find("li").last()), c.addClass("active").find('a').addClass('active');
        },
        listen: function() {
        ...
        mouseenter: function(b) {
            this.mousedover = !0, this.$menu.find(".active").removeClass("active"), a(b.currentTarget).addClass("active").find('a').addClass('active');
        },
